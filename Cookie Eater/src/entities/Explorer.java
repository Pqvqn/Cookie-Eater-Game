...
public class Explorer extends Entity{
	...
	public Explorer(Game frame, Board gameboard, SaveData sd, int cycle) {
		...
		if(sd.getBoolean("residence",0)) {
			residence = board.findFloor(sd.getString("residence",1)).findRoom(sd.getString("residence",2));
		}else {
			residence = board.stores.get(sd.getString("residence",2));
		}
		...
		if(sell_data!=null) {
			for(int i=0; i<sell_data.size(); i++) {
				CookieStore disp;
				to_sell.add(disp = CookieStore.loadFromData(game, board, null, sell_data.get(i)));
				disp.setVendor(this);
			}
		}
	...
	public SaveData getSaveData() {
		...
		data.addData("residence",residence.getFloor()!=null,0);
		data.addData("residence",residence.getFloor()==null?"0":residence.getFloor().getID(),1);
		data.addData("residence",residence.getID(),2);
	...
	public void traverseShop(double xt, double yt, double rt) {
		...
		if(Math.sqrt(Math.pow(x-xt,2)+Math.pow(y-yt,2))<getRadius()+rt){
			...
			dir = y<yt ? DOWN : UP; 
		}else if(Math.abs(y-board.currLevel.y_resol/2)<getRadius()) {
			dir = x>xt ? LEFT : RIGHT; 
		}else {
			dir = y<board.currLevel.y_resol/2 ? DOWN : UP; 
		}
